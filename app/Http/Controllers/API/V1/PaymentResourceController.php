...
class PaymentResourceController extends Controller
{
    ...
    public function pay(PaymentRequest $request){ //this function handles the payment request from your website to paypal.
        ...
        if(strtolower($request->payment_method) == "cod"){
        ...
        }else if(strtolower($request->payment_method) == "paypal"){    
            $order = Order::create([ //create order entry
            ...
            try{
                ...
                if($response->isRedirect()){//if the response (->send()) is successful
                    ...
                    return response($response->getMessage()); // get the error message.
                }
            }catch(\Throwable $th){
                $this->deleteOrder($order->id);
                return response($th->getMessage()); // get/return error message
            }
        }
    }
    ...
    public function error($id){
        ...
        return  response("User declined the payment!");
